...
def get_priority(confidence_score):
...
def send_to_aris(command, confidence=0.95):
    payload = {
        ...
        "priority": get_priority(round(float(confidence) / 100, 2)),
        "status": "validated"
    }
...
def get_hand_hist():
    with open(BASE_PATH / "hist", "rb") as f:
...
hist = get_hand_hist()
model = load_model(str(BASE_PATH / "cnn_model_keras2.h5"))

with open(BASE_PATH / "label_mapping.pkl", "rb") as f:
